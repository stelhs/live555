...
int main(int argc, char** argv) {
  ...
  while (argc > 1) {
    ...
    case 'N': { // specify script to commit videos
      scriptComplete = argv[2];
      ++argv; --argc;
      break;
    }
  ...
  if (outputQuickTimeFile || outputAVIFile) {
    ...
    while ((subsession = iter.next()) != NULL) {
      ...
      if (singleMedium == NULL || periodicFilenameSuffix[0] != '\0') {
        ...
        snprintf(path, sizeof path, "/tmp/%s", fileNamePrefix);
        mkpath(path, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
        snprintf(outFileName, sizeof outFileName, "/tmp/%s/%s-%s-%d",
                 fileNamePrefix, subsession->mediumName(),
                 subsession->codecName(), recCounter);
      } else {
      ...
      if (strcmp(subsession->mediumName(), "video") == 0) {
        strncpy(videoFile, outFileName, sizeof videoFile);
        ...
        strncpy(audioFile, outFileName, sizeof audioFile);
...
void periodicFileOutputTimerHandler(void* /*clientData*/) {
  ...
  if (pid == 0) {
    char timeStr[16];
    printf("call %s %s %d %s %s\n", scriptComplete,
           fileNamePrefix, startRecording, videoFile, audioFile);
    snprintf(timeStr, sizeof timeStr, "%d", startRecording);

    char *const argv[] = {(char *)"", "openRTSP_callback",
                          (char *)fileNamePrefix, timeStr,
                          videoFile, audioFile, NULL};
    char *const env[] = { NULL };
    execve(scriptComplete, argv, env);
    exit(0);
  }
